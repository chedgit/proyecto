...
Public Class cls_BDEvaluacion
            ...
            If cuentaareasrese = "1" Then
                ...
                For w As Integer = 0 To ptcol.PointCount - 2
                    ...
                    lostrRetorno_1 = cls_Oracle.FT_SG_D_COORD_EVALGIS("ACT", pastr_CGCODIGO, IIf(glo_InformeDM = "", "1", glo_InformeDM), pastr_CGCODEVA, lostNumPoligono, v_IECODIGO, _
                              w, lostrCoox, lostrCooy, lointArea, gstrUsuarioAcceso)
                Next w
    ...
    Public Sub InsertarCoordenadas(ByVal pLayer As String, ByVal pastr_CGCODIGO As String, ByVal p_ListBox As Object, ByVal p_App As IApplication, ByRef p_Estado As String)
        ...
        lostrRetorno = cls_Oracle.FT_SG_CUENTA_REG_IN_EVALGIS("SG_D_EVALGIS", pastr_CGCODIGO, glo_InformeDM, "", "PR")
        For i As Integer = 0 To p_ListBox.RowCount - 1
            ...
            If lostrRetorno = "1" Then
                ...
                lostrRetorno1 = cls_Oracle.FT_SG_D_AREAS_EVALGIS("ACT", pastr_CGCODIGO, IIf(glo_InformeDM = "", "1", glo_InformeDM), lostCODEVA, num_area, "PR", _
                lodouArea, "", gstrUsuarioAcceso)
                ...
                For w As Integer = 0 To ptcol.PointCount - 2
                    ...
                    lostrRetorno = cls_Oracle.FT_SG_D_COORD_EVALGIS("ACT", pastr_CGCODIGO, glo_InformeDM, lostCODEVA, num_area, vertices, "PR",
            ...
            Else
                ...
                lostrRetorno1 = cls_Oracle.FT_SG_D_AREAS_EVALGIS("INS", pastr_CGCODIGO, glo_InformeDM, lostCODEVA, num_area, "PR", _
                lodouArea, " ", gstrUsuarioAcceso)
                ...
                For w As Integer = 0 To ptcol.PointCount - 2
                    ...
                    lostrRetorno1 = cls_Oracle.FT_SG_D_COORD_EVALGIS("INS", pastr_CGCODIGO, glo_InformeDM, lostCODEVA, num_area, vertices, "PR", _
                                lostrCoox, lostrCooy, 0, gstrUsuarioAcceso)
    ...
    Public Sub InsertarCoordenadas_2(ByVal pLayer As String, ByVal pastr_CGCODIGO As String, ByVal p_ListBox As Object, ByVal p_App As IApplication, ByRef p_Estado As String)
        ...
        Do Until pFeature Is Nothing
            ...
            lodouArea = pFeature.Value(pFeature.Fields.FindField("AREA_FINAL")).ToString
            ' MsgBox(lodouArea)
            ...
            If var_fa_tipoactualiza = False Then  'Caso insertar por 1er vez Area Disponible
                lostrRetorno = cls_Oracle.FT_SG_D_CARAC_EVALGIS("INS", pastr_CGCODIGO, glo_InformeDM, "1 | " & pastr_CGCODIGO & " ||", "1 | AD ||", _
                "", lodouArea, "", gstrUsuarioAcceso)

                ' lostrRetorno = cls_Oracle.FT_SG_D_CARAC_EVALGIS("INS", pastr_CGCODIGO, glo_InformeDM, pastr_CGCODIGO, "AD", _
                '"", lodouArea, "", gstrUsuarioAcceso)

            ElseIf var_fa_tipoactualiza = True Then  'Caso Actualizacion
                lostrRetorno = cls_Oracle.FT_SG_D_CARAC_EVALGIS("ACT", pastr_CGCODIGO, glo_InformeDM, "1 | " & pastr_CGCODIGO & " ||", "1 | AD ||", _
                "", lodouArea, "", gstrUsuarioAcceso)
                'lostrRetorno = cls_Oracle.FT_SG_D_CARAC_EVALGIS("ACT", pastr_CGCODIGO, glo_InformeDM, pastr_CGCODIGO, "AD", _
                '"", lodouArea, "", gstrUsuarioAcceso)
       ...
       ByVal p_IECodigo As String, ByVal p_App As IApplication)
        ...
        Do Until pFeature Is Nothing
            ...
            For w As Integer = 0 To ptcol.PointCount - 2
                ...
                lostNumPoligono, p_IECodigo, w, lostrCoox, lostrCooy, lointArea, gstrUsuarioAcceso)
    ...
    Public Sub InsertarCoordenadas_Cuadriculas(ByVal pLayer As String, ByVal pastr_CGCODIGO As String, ByVal p_IECodigo As String, _
       ByVal ESTADO As String, ByVal pastr_CGCODEVA As String, ByVal p_App As IApplication)
        Dim lostrRetorno_0 As String = "", lostrRetorno_1 As String = ""
        Dim cls_Oracle As New cls_Oracle
        'Dim v_IECODIGO As String = ""
        pMxDoc = p_App.Document
        ...
        '  Dim p_IECodigo As String
        Do Until pFeature Is Nothing
            lostrCooy = "" : lostrCoox = ""
            lostNumPoligono = pFeature.Value(pFeature.Fields.FindField("POLIGONO")) ', InStr(pFeature.Value(pFeature.Fields.FindField("CODIGOU")), "_") + 1))
            'lodouArea = pFeature.Value(pFeature.Fields.FindField("HAS"))
            lodouArea = pFeature.Value(pFeature.Fields.FindField("Area"))
            tipo = pFeature.Value(pFeature.Fields.FindField("TIPO"))
            ' reserva = pFeature.Value(pFeature.Fields.FindField("RESERVA"))
            'p_IECodigo = "AR"
            l = pFeature.Shape
            pArea = pFeature.Shape
            lointArea = Math.Abs(pArea.Area) / 10000
            ptcol = l
            lointNumVer = ptcol.PointCount - 1
            If tipo = "Area Total" Or tipo = "Area Parcial" Then

                'conta_x = reserva.Length
                'If conta_x = 8 Then
                '    cuenta_proceso = 1
                'ElseIf conta_x = 17 Then
                '    cuenta_proceso = 2
                'End If

                'For a As Integer = 1 To cuenta_proceso
                '    If a = 1 Then
                '        pastr_CGCODEVA = Mid(reserva, 1, 8)

                '    ElseIf a = 2 Then
                '        pastr_CGCODEVA = Mid(reserva, 10, 18)
                '    End If
                Dim vertices As Integer = 0
                For w As Integer = 0 To ptcol.PointCount - 2
                    vertices = vertices + 1
                    ...
                    lostrRetorno_1 = cls_Oracle.FT_SG_D_COORD_EVALGIS(ESTADO, pastr_CGCODIGO, IIf(glo_InformeDM = "", "1", glo_InformeDM), pastr_CGCODEVA, _
                    lostNumPoligono, vertices, p_IECodigo, lostrCoox, lostrCooy, lointArea, gstrUsuarioAcceso)
                    'lostrCoox = ""
                    'lostrCooy = ""
                    'lostrRetorno_1 = cls_Oracle.FT_SG_D_COORD_EVALGIS("INS", pastr_CGCODIGO, glo_InformeDM, pastr_CGCODEVA, _
                    'lodouArea, p_IECodigo, lostrCoox, lostrCooy, lointArea, gstrUsuarioAcceso)
